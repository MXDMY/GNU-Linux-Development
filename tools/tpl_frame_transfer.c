/*******************************************************************************************
 ...
 2026.04 更新：
 采用循环缓冲区，减少内存拷贝与内存分配，减少内存碎片同时避免拷贝开销。生产者收数据放入缓冲区，消费
 者从缓冲区提取数据。采用原子操作隔离，以降低互斥开销。溢出计数通过取余回绕。
 
 2026.06 更新：
 收发保留对循环缓冲区的利用。考虑到不同编译器 C 标准支持程度以及自身特性不同，原子操作语句变化大，选
 择统一生产者与消费者部分功能，不再使用原子操作隔开。接收数据与检出帧全部由生产者负责，并放入帧列表，
 由应用线程选择性提取。

 2026.08 更新：
 将生产者与消费者重新拆开，删除帧列表，查找帧列表时间复杂度高。原子操作要求与 C11 或 GCC 接口对齐
 ，至少支持 C11 或 GCC 的 seq_cst 严格内存顺序实现。受 Linux 内核 kfifo 启发，使用 2^n & 运
 算，替换掉原来计数溢出的 % 运算（虽然只看这部分，其它部分由补码原理自行推算，但应该差不多，更详细
 部分自行翻阅资料）。
*******************************************************************************************/
...
static int tpl_read(void* buf, int size)
{
    ...
    = { 
        ...
        0xBB, 0xAA, 0xFF, 0xFF, 0x10, 0x11, 0x84,
    };
 ...
 * @param start     出发位置
 * @param size      可用字节数
 * @return          完整帧大小；帧数据不够，无法判断长度，则返回 0；
 *                  长度异常（如大于缓冲区长度）等均返回负数
 * @note            注意溢出；注意端序
*/
...
static void tpl_consumer(void)
{
    ...
    while (tpl_atomic_load(tpl_run))
    {
        ...
        if (frame_size <= 0)
        {
            ...
            if (frame_size < 0 || frame_size >= TPL_BUF_SIZE)
            {
                tpl_pr("{%s}invalid frame size\n", __func__);
                tpl_atomic_add(tpl_i1, 1);
                is_searched = 0;
                frame_size = -1;
            }
            continue;
